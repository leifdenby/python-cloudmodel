...
parser.add_argument('-dT_b', help="bubble temperature perturbation", default=0.1, type=float)
...
beta = args.beta
model = ModelClass(environment=environment, gamma=0.2, beta=beta, D=0.1, microphysics=microphysics)
z_max = args.z_max
N_clouds = args.N
z = np.linspace(z_clb, args.z_max, (z_max - z_clb)/10.)
...
profiles = []
for r_c in np.linspace(100., r_max, N_clouds, endpoint=True):
...
pyclouds.plotting.plot_profiles(profiles, ['r', 'w', 'T', 'q_v', 'q_l', 'q_r'], label_f=label_f, labels_ncol=6)
title = """Integration from LCL in {environment}
model: {model}
$dT_{{bubble}}={dT_b}K$, $dq_{{v,bubble}}={dqv_b}kg/kg$, $z_{{clb}}={z_clb}m$
integrator: {integrator}
""".format(environment=environment, model=str(model), dT_b=dT_b, 
        dqv_b=dqv_b, integrator=str(integrator), z_clb=z_clb)
plot.suptitle(title)
plot.show()
