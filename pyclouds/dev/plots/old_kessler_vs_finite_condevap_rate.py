...
for dt in [t_max/50, t_max/100, t_max/200]:
    ...
    for q_l in [1.0e-3, 1.0e-4, 1.0e-5]:
        ...
        solutions.append(cloud_microphysics.FiniteCondensationTimeMicrophysics(constants=constants).integrate(initial_condition=initial_condition, t=t_, p0=p0))

        plot_hydrometeor_evolution(solutions, variables=['q_l', 'T', ], legend_loc='upper right')
        ...
        title += "\n"+ r"with $c_{p,d}=%g\frac{J}{kg K}$, $L_v=%gJ/kg$ expect $\Delta T\approx \frac{L_v q_l}{c_{p,d}}=%gK$" % (cp_d, L_v, dT_approx)
        title += '\n'+ r"$\Delta t=%gs$" % dt
