...
t = numpy.linspace(0, 1800., 600)
cloud_model = cloud_equations.DryAirOnly(gamma=gamma, D=D, beta=beta, environment=environment)
profiles.append(cloud_model.integrate_in_time(initial_condition, t))

t = numpy.linspace(0, 1000., 600)
cloud_model = cloud_equations.DryAirOnly(gamma=gamma, D=D, beta=0.2, environment=environment)
profiles.append(cloud_model.integrate_in_time(initial_condition, t))

fig = plotting.plot_profiles(profiles, variables=['r', 'w', 'T', 'q_v', 'T__tephigram', ], initial_condition=initial_condition)
