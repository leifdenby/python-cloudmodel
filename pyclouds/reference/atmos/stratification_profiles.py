...
def getStandardIsothermalAtmosphere():
    ...
    dTdz = 0.0  # isothermal
    g = scipy.constants.g
    return HydrostaticallyBalancedAtmosphere(
        rho0=rho0, p0=p0, dTdz=dTdz, gas_properties=gas_properties, g=g
    )
...
class HydrostaticallyBalancedAtmosphere(object):
    ...
    def __str__(self):
        return "HydrostaticallyBalancedAtmosphere (rho0=%f, p0=%f, dTdz=%f) with %s" % (
            self.rho0,
            self.p0,
            self.dTdz,
            str(self.gas_properties),
        )
...
class PseudoHydrostaticallyBalancedMoistAtmosphere(HydrostaticallyBalancedAtmosphere):
    ...
    def __init__(self, rho0, p0, dTdz, RH0, dRHdz, gas_properties, g=None):
        super(PseudoHydrostaticallyBalancedMoistAtmosphere, self).__init__(
            rho0=rho0, p0=p0, dTdz=dTdz, gas_properties=gas_properties, g=g
        )
...
class LayeredAtmosphere(object):
    ...
    def _get_values_from_layer(self, variable, pos):
        ...
        if type(pos) in [float, np.float, np.float64]:
            ...
            for (z_min, z_max), layer in list(self.layer_instances.items()):
...
class NearIsentropic(HydrostaticallyBalancedAtmosphere):
    ...
    def __init__(self, dTdz_offset=1.0e-3):
        ...
        super(NearIsentropic, self).__init__(
            rho0=rho0, p0=p0, dTdz=dTdz, gas_properties=gas_properties, g=g
        )
    ...
    def __str__(self):
        return "Near-isentropic, dTdz_offset={offset}K/km (dry)".format(
            offset=self.dTdz_offset * 1.0e3
        )
...
class Soong1973(LayeredMoistAtmosphere):
    def __init__(self, cloud_base_height=None):
        ...
        if self.cloud_base_height is not None:
            ...
            if cloud_base_height == 1600.0:
                RH0 = 0.50
            elif cloud_base_height == 1100.0:
                RH0 = 0.59
            else:
        ...
        layers.append({"z_max": 12800.0, "dTdz": dTdz_moist, "dRHdz": dRHdz_1})
        ...
        super(Soong1973, self).__init__(
            layers=layers, RH0=RH0, RH_min=0.3, rho0=rho0, p0=p0
        )
    ...
    def __str__(self):
        if self.cloud_base_height is None:
            ...
            return (
                "Soong 1973 layered moist atmosphere (with modified cloud base at %s)"
...
class SimpleMoistStable(LayeredMoistAtmosphere):
    ...
    def __str__(self):
        return "Simple stable moist atmosphere based on Soong1973 (dRHdz=%g%%/km)" % (
            self.dRHdz * 1.0e3
        )
...
class RICO:
    ...
    def __init__(self, include_wind=False):
        ...
        # surface conditions
        self.ps = 101540.0  # [Pa], surface pressure
        self.p0 = 100000.0  # [Pa], reference pressure
    ...
    def iteratively_find_temp(self, theta_l, p, q_t, q_l, T_initial):
        ...
        theta_l_f = (
            ...
            * (p0 / p) ** (R_l / c_l)
            * omega_l
            * np.exp(-L_v * q_l / (c_l * T))
        )

        f = lambda T: C + np.log(T) - L_v * q_l / (c_l * T)
        T = scipy.optimize.brentq(f, 1.0, 410.0)
    ...
    def __str__(self):
        return (
            "RICO, LES test case from KNMI (%s wind)"
            % ["without", "with"][self.include_wind]
        )
...
class DiscreteProfile:
    ...
    def __init__(self, z, description, **kwargs):
        ...
        if "T" in kwargs and "temp" not in kwargs:
            self.vars["temp"] = kwargs["T"]

        if "temperature" in kwargs and "temp" not in kwargs:
    ...
    def __getattr__(self, name):
        if name in self.__dict__:
            ...
            def f(z):
                return scipy.interpolate.interp1d(x=self.z, y=y_discrete)(z)
...
class TwoLayerMoistIsentropicPBL:
    ...
    def __init__(
        ...
        gas_properties = AttrDict(M=scipy.constants.R * 1000.0 / R)
    ...
    def __str__(self):
        return (
            "Idealised Two-layer atmosphere ($RH_0={RH0:.0f}\%$, "
            "$T_0={T0}K$, $z_{{BL}}={z_bl}m$, $z_{{INV}}={z_INV}m$)".format(
                RH0=self.RH0 * 100.0, T0=self.T0, z_bl=self.z_BL, z_INV=self.z_INV
            )
        )
